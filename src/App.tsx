...
function App() {
  const [lesson, setLesson] = useState<1 | 2 | 3 | 4 | 5 | 6 | 7 | 8>(8)
  const [menuOpen, setMenuOpen] = useState(false)
  const menuButtonRef = useRef<HTMLButtonElement>(null)
  const focusMode = true
  function chooseLesson(value: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8) {
    setLesson(value)
    setMenuOpen(false)
    menuButtonRef.current?.focus()
  }
  const lessonNavigation = <nav className="preview-lesson-nav" aria-label="Preview lesson">
    {([1, 2, 3, 4, 5, 6, 7, 8] as const).map(value => <button key={value} type="button" className={lesson === value ? 'is-active' : ''} onClick={() => chooseLesson(value)}>Lesson {value}</button>)}
  </nav>
  return (
    <div className={`app-shell app-shell--lesson${focusMode ? ' app-shell--study' : ''}`} onKeyDown={event => {
      ...
      <main className="lesson-preview">
        {lesson === 1 ? (
          ...
          />
        ) : lesson === 2 ? <OperationsVariantCLesson /> : lesson === 3 ? <TutorPlaceValueLesson /> : lesson === 4 ? <TutorLongMultiplicationLesson /> : lesson === 5 ? <TutorLongDivisionLesson /> : lesson === 6 ? <TutorDecimalsLesson /> : lesson === 7 ? <TutorFactorsLesson /> : <TutorFractionsLesson />}
      </main>
    </div>
  )
}
