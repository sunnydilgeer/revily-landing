...
export function ScienceVariantSwitch({ variant, lessonNumber, activity }: { variant: ScienceVariant; lessonNumber?: LessonNumber; activity?: string | null }) {
  if (lessonNumber && lessonNumber >= 7) return <div className="science-variant-bar"><nav aria-label="Lesson wording" className="science-variant-switch"><span aria-current="page">Easier wording · only version</span></nav><p>Lessons 7–26 were authored once in the clearer wording style. There is no baseline copy.</p></div>
  return <div className="science-variant-bar">
    ...
    <p>Lessons 1–6 have both wording choices and separate progress. Lessons 7–26 use easier wording only.</p>
  </div>
}
