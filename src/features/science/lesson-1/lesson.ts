...
export const lesson1: ScienceLesson = {
  id: 'B-CELL-001', contentVersion: '0.3.0', qualification: 'AQA-8464F', strand: 'biology',
  ...
  states: [
    q({ id: 'B1-01', phase: 'priorKnowledge', title: 'What is a cell?', contextId: 'ks3-baseline', dimensions: ['recall'], evidenceRole: 'diagnostic',
      options: [{ id: 'unit', label: 'A basic unit of living organisms' }, { id: 'organ', label: 'An organ, such as the heart' }, { id: 'organism', label: 'Always a whole animal' }], answerId: 'unit', answer: 'A basic unit of living organisms.', hint: 'An organ contains many smaller living units.', reasoning: ['Living organisms are made of cells.', 'An organ contains many cells; a cell is not the same as an organ.'] }),
    { id: 'B1-02', kind: 'teaching', phase: 'teach', skillId, specRefs, sourceIds, title: 'Animal cells: meet the cell', visual: model(),
      ...
      body: 'Explore a typical animal cell: its membrane, cytoplasm and nucleus.' },
    q({ id: 'B1-03', phase: 'model', title: 'The pointer touches the outer boundary. Name the part.', visual: model('boundary'), contextId: 'model-a-boundary', dimensions: ['recall'], evidenceRole: 'practice', options: parts.slice(0, 3), answerId: 'membrane', answer: 'Cell membrane.', hint: 'Trace the outside edge. Do not select an internal structure.', reasoning: ['The pointer ends on the cell’s outer boundary.', 'In this animal-cell model, that boundary is the cell membrane.'] }),
    q({ id: 'B1-04', phase: 'guided', title: 'A substance enters the cell. Which part controls entry?', contextId: 'entry-guided', dimensions: ['understanding'], evidenceRole: 'practice', options: parts.slice(0, 3), answerId: 'membrane', answer: 'Cell membrane.', hint: 'Link the boundary to movement into or out of the cell.', reasoning: ['The cell membrane controls movement of substances into and out of the cell.', 'The nucleus controls activities, not the boundary crossing in this question.'] }),
    ...
    q({ id: 'B1-06', phase: 'misconception', title: 'Which statement is scientifically correct?', contextId: 'energy-claim', dimensions: ['understanding'], evidenceRole: 'practice',
      options: [{ id: 'release', label: 'Respiration releases energy for cell processes.' }, { id: 'create', label: 'Mitochondria create energy from nothing.', misconceptionSignal: 'MC-ENERGY-CREATED', feedback: 'This statement confuses energy release with energy creation.' }, { id: 'protein', label: 'Ribosomes carry out aerobic respiration.', misconceptionSignal: 'MC-RIBOSOME-RESPIRATION' }], answerId: 'release', answer: 'Respiration releases energy for cell processes.', hint: 'Think about release, not creation; distinguish respiration from protein synthesis.', reasoning: ['Mitochondria are the site of aerobic respiration.', 'Respiration releases energy; ribosomes make proteins.'] }),
    { id: 'B1-07', kind: 'teaching', phase: 'guided', skillId, specRefs, sourceIds: [...sourceIds, 'aqa-2023-ms'], title: 'Build a linked explanation',
      body: 'A cell needs energy for its work. How do mitochondria help?',
      steps: ['Name the structure: mitochondria.', 'State the process: aerobic respiration.', 'Link the function: respiration releases energy for the cell’s work.'] },
    ...
    q({ id: 'B1-09', phase: 'misconception', title: 'Our model shows a purple nucleus. What does that tell us?', visual: model(), contextId: 'model-colour', dimensions: ['understanding'], evidenceRole: 'practice', sourceIds: ['aqa-ws'], specRefs: ['WS 1.2'],
      options: [{ id: 'key', label: 'Purple identifies the nucleus in this model.' }, { id: 'natural', label: 'All living nuclei are naturally purple.', misconceptionSignal: 'MC-MODEL-COLOUR' }, { id: 'function', label: 'Purple is what lets the nucleus control activities.' }], answerId: 'key', answer: 'Purple identifies the nucleus in this model.', hint: 'Is this a photograph or a representational diagram?', reasoning: ['Schematic colours help distinguish parts.', 'A model colour is not evidence of the natural colour of a living structure.'] }),
    { id: 'B1-10', kind: 'teaching', phase: 'practicalConnection', skillId, specRefs: ['4.1.1.2', 'WS 1.2', 'WS 3.5'], sourceIds: ['aqa-practical', 'aqa-ws'], title: 'Drawings should follow observations',
      body: 'A school light-microscope view may show less detail than this model. Record visible features; do not add dots just because a textbook has them. Not visible in a view does not mean absent from a cell. Later you will learn microscope technique, drawings and magnification.',
    ...
    q({ id: 'B1-11', phase: 'practicalConnection', title: 'Tiny ribosomes were not distinguished in this light-microscope view. Which conclusion is justified?', contextId: 'unseen-view', dimensions: ['practicalReasoning', 'dataInterpretation'], evidenceRole: 'practice', sourceIds: ['aqa-ws', 'aqa-practical'], specRefs: ['WS 1.2', 'WS 3.5'],
      options: [{ id: 'limit', label: 'This view does not establish whether ribosomes are absent.' }, { id: 'absent', label: 'The cell definitely has no ribosomes.', misconceptionSignal: 'MC-UNSEEN-ABSENT' }, { id: 'all', label: 'Every animal cell must lack ribosomes.' }], answerId: 'limit', answer: 'This view does not establish whether ribosomes are absent.', hint: 'Separate what was seen from what exists.', reasoning: ['The observation says tiny structures were not distinguished.', 'That is a limit of this view, not proof that the cell lacks them.'] }),
    ...expandedTeaching,
    ...([
      ['12', 'Where is most genetic material enclosed in a typical animal cell?', 'nucleus', 'Nucleus.', 'The nucleus contains genetic material and controls cell activities.'],
      ...
      ['16', 'Which structures synthesise proteins?', 'ribosomes', 'Ribosomes.', 'Protein synthesis takes place at ribosomes.'],
    ].map(([n, title, answerId, answer, reason]) => q({ id: `B1-${n}`, phase: 'independent', title, contextId: `function-recall-${answerId}`, dimensions: ['recall'], evidenceRole: 'independent', options: parts, answerId, answer, hint: 'Recall the five functions from the teaching; this hint does not identify a part.', reasoning: [reason], exam: { marks: 1, ao: 'AO1', status: 'revilyDraft' } }))),
    q({ id: 'B1-17', phase: 'independent', title: 'Which explanation correctly distinguishes nucleus and membrane?', contextId: 'control-distinction', dimensions: ['understanding'], evidenceRole: 'independent',
      options: [{ id: 'different', label: 'The nucleus controls cell activities; the membrane controls entry and exit.' }, { id: 'same', label: 'Both names mean the same structure.' }, { id: 'reversed', label: 'The membrane holds the nucleus’s genetic material; the nucleus is the outer boundary.' }], answerId: 'different', answer: 'The nucleus controls activities; the membrane controls entry and exit.', hint: 'The word “controls” can refer to different functions.', reasoning: ['The nucleus and membrane are different structures.', 'Their control roles concern cell activities and boundary movement respectively.'], exam: { marks: 1, ao: 'AO1', status: 'revilyDraft' } }),
    q({ id: 'B1-18', phase: 'independent', title: 'Which explanation correctly distinguishes mitochondria and ribosomes?', contextId: 'process-distinction', dimensions: ['understanding'], evidenceRole: 'independent',
      options: [{ id: 'jobs', label: 'Mitochondria: aerobic respiration. Ribosomes: protein synthesis.' }, { id: 'swapped', label: 'Mitochondria: protein synthesis. Ribosomes: aerobic respiration.', misconceptionSignal: 'MC-RIBOSOME-RESPIRATION' }, { id: 'created', label: 'Both structures create energy.', misconceptionSignal: 'MC-ENERGY-CREATED' }], answerId: 'jobs', answer: 'Mitochondria: aerobic respiration. Ribosomes: protein synthesis.', hint: 'Match a process to each part, rather than relying on where it was drawn.', reasoning: ['Aerobic respiration and protein synthesis are different processes.', 'Mitochondria and ribosomes perform these respective functions.'], exam: { marks: 1, ao: 'AO1', status: 'revilyDraft' } }),
    q({ id: 'B1-19', phase: 'transfer', title: 'A gland cell makes proteins for release. Which structure is directly involved in making them?', contextId: 'gland-protein', dimensions: ['application'], evidenceRole: 'independent', options: parts, answerId: 'ribosomes', answer: 'Ribosomes.', hint: 'Use the given task: making proteins. No prior knowledge of glands is needed.', reasoning: ['The question supplies the cell’s job: making proteins.', 'Ribosomes carry out protein synthesis.'], exam: { marks: 1, ao: 'AO2', status: 'revilyDraft' } }),
    ...
    { id: 'B1-21', kind: 'written', phase: 'transfer', skillId, specRefs: ['4.1.1.1', '4.1.1.2'], sourceIds: ['aqa-biology'],
      title: 'Give two structural differences between a typical animal cell and a bacterial cell.', contextId: 'animal-bacterial-comparison', dimensions: ['explanation'], evidenceRole: 'independent', hint: 'Write two paired comparisons: an animal cell has…, whereas a bacterial cell… .', marking: 'teacherOnly',
  ...
  retrieval: [
    ...
    q({ id: 'B1-R03', phase: 'retrieval', title: 'A cell makes a protein used elsewhere in the body. Which structure makes that protein?', contextId: 'delayed-protein-application', dimensions: ['application'], evidenceRole: 'delayed', options: parts, answerId: 'ribosomes', answer: 'Ribosomes.', hint: 'Which process makes proteins?', reasoning: ['The unfamiliar context still specifies protein synthesis.', 'Ribosomes perform that process.'] }),
    { id: 'B1-R04', kind: 'written', phase: 'retrieval', skillId, specRefs: ['4.1.1.1', '4.1.1.2'], sourceIds, title: 'Compare two structures in a typical animal cell with those in a bacterial cell.', contextId: 'delayed-written-comparison', dimensions: ['explanation'], evidenceRole: 'delayed', hint: 'Use two distinct paired structural comparisons.', marking: 'teacherOnly',
