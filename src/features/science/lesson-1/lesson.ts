...
export const lesson1: ScienceLesson = {
  ...
  states: [
    q({ id: 'B1-01', phase: 'priorKnowledge', title: 'What is a cell?', contextId: 'ks3-baseline', dimensions: ['recall'], evidenceRole: 'diagnostic',
      options: [{ id: 'unit', label: 'A basic living unit that organisms are made of' }, { id: 'organ', label: 'An organ, such as the heart' }, { id: 'organism', label: 'Always a whole animal' }], answerId: 'unit', answer: 'A basic living unit that organisms are made of.', hint: 'An organ contains many smaller living units.', reasoning: ['Living organisms are made of cells.', 'An organ contains many cells; a cell is not the same as an organ.'] }),
    ...
    q({ id: 'B1-04', phase: 'guided', title: 'A substance enters the cell. Which part controls entry?', contextId: 'entry-guided', dimensions: ['understanding'], evidenceRole: 'practice', options: parts.slice(0, 3), answerId: 'membrane', answer: 'Cell membrane.', hint: 'Think about which part separates the cell from its surroundings and controls what crosses that boundary.', reasoning: ['The cell membrane controls movement of substances into and out of the cell.', 'The nucleus controls cell activities. The membrane has a different job: controlling which substances enter and leave.'] }),
    ...
    q({ id: 'B1-06', phase: 'misconception', title: 'Which statement is scientifically correct?', contextId: 'energy-claim', dimensions: ['understanding'], evidenceRole: 'practice',
      options: [{ id: 'release', label: 'Respiration releases energy for cell processes.' }, { id: 'create', label: 'Mitochondria create energy from nothing.', misconceptionSignal: 'MC-ENERGY-CREATED', feedback: 'This statement confuses energy release with energy creation.' }, { id: 'protein', label: 'Ribosomes carry out aerobic respiration.', misconceptionSignal: 'MC-RIBOSOME-RESPIRATION' }], answerId: 'release', answer: 'Respiration releases energy for cell processes.', hint: 'Respiration releases energy from food. Protein synthesis means making proteins. Which statement uses these ideas correctly?', reasoning: ['Mitochondria are the site of aerobic respiration.', 'Respiration releases energy; ribosomes make proteins.'] }),
    { id: 'B1-07', kind: 'teaching', phase: 'guided', skillId, specRefs, sourceIds: [...sourceIds, 'aqa-2023-ms'], title: 'Explain how a cell structure helps',
      ...
      steps: ['Name the structure: mitochondria.', 'Name what happens there: aerobic respiration, which uses oxygen.', 'Link the function: respiration releases energy for the cell’s work.'] },
    ...
    q({ id: 'B1-09', phase: 'misconception', title: 'Our model shows a purple nucleus. What does that tell us?', visual: model(), contextId: 'model-colour', dimensions: ['understanding'], evidenceRole: 'practice', sourceIds: ['aqa-ws'], specRefs: ['WS 1.2'],
      options: [{ id: 'key', label: 'Purple identifies the nucleus in this model.' }, { id: 'natural', label: 'All living nuclei are naturally purple.', misconceptionSignal: 'MC-MODEL-COLOUR' }, { id: 'function', label: 'Purple is what lets the nucleus control activities.' }], answerId: 'key', answer: 'Purple identifies the nucleus in this model.', hint: 'Is this a photograph of a real cell, or a diagram using colours to identify parts?', reasoning: ['This diagram uses different colours so you can tell the parts apart.', 'A model colour is not evidence of the natural colour of a living structure.'] }),
    { id: 'B1-10', kind: 'teaching', phase: 'practicalConnection', skillId, specRefs: ['4.1.1.2', 'WS 1.2', 'WS 3.5'], sourceIds: ['aqa-practical', 'aqa-ws'], title: 'Drawings should follow observations',
      body: 'A microscope view shows what you can observe, whereas this model shows cell structures for learning. When making a practical drawing, record only features you can distinguish in the microscope view. A structure might be present but too small or unclear to see. You will practise observing and drawing cells in Lesson 3.',
    ...
    q({ id: 'B1-11', phase: 'practicalConnection', title: 'Ribosomes could not be seen clearly in this light-microscope view. What can you conclude?', contextId: 'unseen-view', dimensions: ['practicalReasoning', 'dataInterpretation'], evidenceRole: 'practice', sourceIds: ['aqa-ws', 'aqa-practical'], specRefs: ['WS 1.2', 'WS 3.5'],
      options: [{ id: 'limit', label: 'We cannot tell from this view whether the cell lacks ribosomes.' }, { id: 'absent', label: 'The cell definitely has no ribosomes.', misconceptionSignal: 'MC-UNSEEN-ABSENT' }, { id: 'all', label: 'Every animal cell must lack ribosomes.' }], answerId: 'limit', answer: 'We cannot tell from this view whether the cell lacks ribosomes.', hint: 'Separate what was seen from what exists.', reasoning: ['The observation only tells us that the tiny structures could not be seen clearly.', 'That is a limit of this view, not proof that the cell lacks them.'] }),
    ...expandedTeaching,
    ...([
    ...
    ].map(([n, title, answerId, answer, reason]) => q({ id: `B1-${n}`, phase: 'independent', title, contextId: `function-recall-${answerId}`, dimensions: ['recall'], evidenceRole: 'independent', options: parts, answerId, answer, hint: 'Think about each part’s job. Match the job in the question to a structure, rather than to its colour or position.', reasoning: [reason], exam: { marks: 1, ao: 'AO1', status: 'revilyDraft' } }))),
  ...
  retrieval: [
    ...
    q({ id: 'B1-R03', phase: 'retrieval', title: 'A cell makes a protein used elsewhere in the body. Which structure makes that protein?', contextId: 'delayed-protein-application', dimensions: ['application'], evidenceRole: 'delayed', options: parts, answerId: 'ribosomes', answer: 'Ribosomes.', hint: 'Which process makes proteins?', reasoning: ['Although the protein is used elsewhere, the cell’s task is still to make a protein.', 'Ribosomes perform that process.'] }),
    { id: 'B1-R04', kind: 'written', phase: 'retrieval', skillId, specRefs: ['4.1.1.1', '4.1.1.2'], sourceIds, title: 'Compare two structures in a typical animal cell with those in a bacterial cell.', contextId: 'delayed-written-comparison', dimensions: ['explanation'], evidenceRole: 'delayed', hint: 'For each comparison, state what the animal cell has and whether the bacterial cell has it. Choose two different structures.', marking: 'teacherOnly',
