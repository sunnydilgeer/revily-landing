...
export default function ScienceLessonHub() {
  ...
  return <div className="science-preview science-preview--revision">
    ...
    <main className="science-course">
      <div className="science-topic"><div><span className="science-eyebrow">Biology · B1</span><h1>Cell biology</h1></div></div>
      <nav aria-label="Science lessons" className="science-course__path"><ol>{scienceLessons.map((item, i) => {
        ...
        return <li key={item.number}><Link className={`science-course__lesson science-course__lesson--${item.number}`} href={scienceLessonHref(item.number)} aria-label={`Lesson ${item.number}: ${item.title}`}>
