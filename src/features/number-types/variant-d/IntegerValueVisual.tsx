...
export function IntegerValueVisual(props: Props) {
  ...
  return <figure className="d-stage">
    ...
    {props.kind === 'openInterval' && <><Line min={6} max={7} insetLabel point={props.revealed ? props.answerValue ?? 6.5 : undefined} label={props.answerValue === undefined ? '6.5' : Number.isInteger(props.answerValue * 10000) ? String(props.answerValue) : 'Your value'} />{props.revealed && <figcaption>{props.answerValue === undefined ? 'One example: 6 < 6.5 < 7' : 'Your value is inside the interval.'}</figcaption>}</>}
