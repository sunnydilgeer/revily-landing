...
function Worked({ root = false }: { root?: boolean }) {
  ...
  const evidence = root ? irrational ? '3² = 9 < 11 < 16 = 4²' : '7 × 7 = 49, so √49 = 7' : '12 ÷ 4 = 3'
  return <>
    ...
    <div className="d-worked__reveal" aria-live="polite">
      {step === 0 ? <p>{root ? 'Which number squared gives this value?' : 'The fraction bar means divide.'}</p> : <p className="d-equation">{evidence}</p>}
      {step === 2 && <><Line min={root ? irrational ? 3 : 5 : 1} max={root ? irrational ? 4 : 9 : 5} point={value} label={root && irrational ? '√11' : String(value)} /><p className="d-conclusion">{irrational && root ? '3 < √11 < 4 → non-integer' : `${expression} = ${value} → integer`}</p></>}
    </div>
...
export function IntegerValueVisual(props: Props) {
  ...
  return <figure className="d-stage">
    ...
    {props.kind === 'rootCheck' && <><div className="d-expression">√{props.radicand}</div>{props.revealed && <><p className="d-equation">{props.radicand === 81 ? '9 × 9 = 81 → √81 = 9' : '4² = 16 < 20 < 25 = 5²'}</p><Line min={props.lower} max={props.upper} point={Math.sqrt(props.radicand)} label={props.radicand === 81 ? '9' : '√20'} /></>}</>}
  </figure>
}
